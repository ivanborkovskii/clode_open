/**
 * Админка: подтверждение удаления, адрес из заголовка, кнопки разметки
 * над текстом статьи и добавление строк для вопросов.
 *
 * Без скрипта админка тоже работает: адрес составится на сервере, теги
 * можно написать руками, подтверждение удаления просто не появится,
 * а вопросы добавляются в две запасные строки, которые есть в форме
 * всегда.
 */
...
(function () {
  ...
  toolbar.addEventListener('click', function (event) {
    ...
    if (button.hasAttribute('data-link')) {
      // Подсказка в вопросе, а не заготовка «https://» в поле: ссылку
      // на свою страницу правильнее писать от косой черты. Абсолютный
      // адрес своего сайта сервер всё равно укоротит при сохранении,
      // но лучше сразу писать как надо.
      var href = window.prompt('Адрес ссылки. На свою страницу — так: /uslugi', '');
  ...
  /* ------------------------------------------------------------------
     Вопросы и ответы: ещё одна строка
     ------------------------------------------------------------------ */

  var faq = document.querySelector('[data-faq]');
  var faqAdd = document.querySelector('[data-faq-add]');

  if (faq && faqAdd) {
    faqAdd.addEventListener('click', function () {
      var rows = faq.querySelectorAll('[data-faq-row]');
      var copy = rows[rows.length - 1].cloneNode(true);

      // Копируем пустую строку, а не последнюю заполненную: иначе новый
      // вопрос приходил бы с чужим текстом внутри.
      copy.querySelectorAll('input, textarea').forEach(function (field) {
        field.value = '';
      });

      faq.appendChild(copy);
