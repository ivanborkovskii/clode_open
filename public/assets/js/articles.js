/**
 * Раздел «Статьи»: подсказки поиска, оценка статьи и плавное раскрытие
 * вопросов под ней.
 *
 * Всё это надстройка. Без скрипта поиск работает как обычная форма,
 * звёзды — как обычные кнопки отправки, а вопросы открываются сразу,
 * без плавности: они сделаны на <details>, который умеет это сам.
 */
...
(function () {
  ...
  /* ------------------------------------------------------------------
     ...
     Сам <details> открывается рывком: пока он закрыт, высоты у него нет
     и анимировать нечего. Поэтому переход берём на себя — отменяем
     нажатие, ставим высоту обёртки вручную и отдаём её CSS.

     Высота считается каждый раз заново, а не запоминается: ответ может
     занять другое число строк после поворота телефона или смены размера
     шрифта.
     ------------------------------------------------------------------ */

  document.querySelectorAll('.faq__item').forEach(function (item) {
    var head = item.querySelector('.faq__head');
    var wrap = item.querySelector('.faq__wrap');
    ...
    /*
     * Досмотреть переход до конца и прибраться.
     *
     * Ждём двумя способами сразу, и срабатывает тот, что успеет первым.
     * Событие «переход закончился» приходит не всегда: его нет, когда
     * в системе выключены анимации, — а без него вопрос остался бы
     * навсегда закрытым для повторного нажатия и не закрывался бы вовсе.
     */
    var after = function (action) {
      var done = function () {
        ...
        // Высоту снимаем: дальше пусть считается сама, по содержимому.
        wrap.style.height = '';
        busy = false;
        action();
      };
    ...
    head.addEventListener('click', function (event) {
      // Браузер открыл бы и закрыл блок мгновенно — делаем это сами.
      event.preventDefault();

      // Быстрые повторные нажатия во время перехода пропускаем, иначе
      // высота застревает на промежуточном значении.
      if (busy) {
        return;
      }
      ...
      if (item.open) {
        // Закрываем: от нынешней высоты к нулю, и только потом сам блок.
      ...
      // Открываем: сначала показываем содержимое, потом тянем высоту
      // от нуля до настоящей.
      item.open = true;
      wrap.style.height = '0px';
