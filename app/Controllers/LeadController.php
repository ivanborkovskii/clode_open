...
final class LeadController extends Controller
{
    ...
    private function mail(array $data, bool $suspicious = false): bool
    {
        ...
        $headers = [
            // Отправителем ставим название компании, а не адрес сайта:
            // так письмо узнаётся в списке входящих с одного взгляда.
            'From: =?UTF-8?B?' . base64_encode('Сайт ' . $this->config['company']['brand'])
                . "?= <{$from}>",
    ...
    private function respond(bool $isAjax, string $status, array $errors, array $values = []): void
    {
        ...
        $this->redirect(
            $this->backUrl() . '?zayavka=' . ($status === 'success' ? 'ok' : 'error') . '#zayavka',
            303,
        );
    }
     ...
     *
     * Берётся из скрытого поля формы. Принимаем только адрес внутри сайта:
     * иначе через это поле можно было бы увести посетителя на чужой сайт.
     */
    private function backUrl(): string
    {
        $back = (string) ($_POST['_back'] ?? '/');

        // Начинается с одного слэша, без протокола и без второго слэша —
        // значит это путь внутри сайта, а не внешняя ссылка.
        if (!preg_match('~^/[a-z0-9\-/]*$~i', $back)) {
            return '/';
        }
