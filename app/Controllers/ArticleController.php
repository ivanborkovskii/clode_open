...
final class ArticleController extends Controller
{
    ...
    private function listing(?array $category): void
    {
        ...
        // же содержимого появлялось бесконечно много адресов, и каждый
        // из них поисковик считал настоящей страницей. Теперь такой адрес
    ...
    public function show(string $slug): void
    {
        ...
        if ($article === null) {
            // Статьи по этому адресу нет — но, может быть, она здесь жила
            // раньше, а адрес поменяли. Тогда переводим на нынешний: иначе
            // человек, пришедший по ссылке из поиска или из закладок,
            // упрётся в пустую страницу, а поисковик выбросит статью
            // вместе со всем, что она успела накопить.
            //
            // Код 301 — «переехали навсегда»: по нему поисковик переносит
            // накопленное со старого адреса на новый.
            $moved = $articles->movedTo($slug);

            if ($moved !== null) {
                $this->redirect('/stati/' . $moved, 301);
