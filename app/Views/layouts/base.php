...
<head>
    ...
    <?php endif; ?>
    <?php
    // «Не бери в индекс, но по ссылкам ходи».
    //
    // Вторая половина важнее первой. Раньше здесь стояло nofollow,
    // и это значило «дальше не ходи»: со страницы, закрытой от индекса,
    // поисковик не переходил по ссылкам. А закрыты у нас списки статей
    // со второй страницы и далее — то есть единственный путь к статьям,
    // которые не поместились на первую, был для него перекрыт.
    //
    // Закрывать от индекса и обрывать обход — разные задачи, и решать
    // их одной строкой нельзя. Собственная страница админки закрыта
    // отдельно, в своём макете, и там nofollow уместно.
    ?>
    ...
    // в корне и отдавался — браузеры находят его там по давнему соглашению,
    // даже если он нигде не объявлен. Но робот, который читает разметку,
    // а не гадает по корню, видел один SVG. Насколько уверенно с ним
    // работает поисковый робот Яндекса, проверить неоткуда, а ICO понимают
    // все без исключения — поэтому он объявлен первым и явно.
    //
    // apple-touch-icon — для значка на экране телефона. Фон у него
    // непрозрачный: iOS скругляет углы сама и подкладывает под прозрачные
    // места чёрный.
    ?>
...
<body>
    ...
    //
    // ВНИМАНИЕ: код счётчика вставляется НЕ СЮДА, а в отдельный файл
    // app/Views/partials/counters.php. Этот макет приходит с обновлениями
    // сайта и при выкладке заменяется целиком — вставленное в него
    // пропадёт. Файл counters.php в обновления не входит никогда,
    // поэтому счётчик в нём переживает любую выкладку.
    ?>
